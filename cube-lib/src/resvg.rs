...
";
static GLOBAL_FONTDB: Lazy<Arc<fontdb::Database>> = Lazy::new(|| {
    ...
    fontdb.set_cursive_family(r"Yellowtail");
    fontdb.set_fantasy_family(r"Sedgwick Ave Display");
    fontdb.set_monospace_family("Noto Mono");
    Arc::new(fontdb)
});

pub fn render(svg: String) -> String {
    let mut opt = usvg::Options::default();
    opt.fontdb = GLOBAL_FONTDB.clone();
    let tree = {
        match usvg::Tree::from_str(svg.as_str(), &opt) {
            Ok(tree) => tree,
    ...
    pixmap.save_png("tests.png").unwrap();
    let data = match pixmap.encode_png() {
        Ok(data) => data,
        _ => return FAIL_IMAGE.to_string(),
    };
    let engine = base64::engine::general_purpose::STANDARD;
    let bas64_string = base64::Engine::encode(&engine, data);
    let bas64_png = format!("data:image/png;base64,{}", bas64_string);
    // let mut file = fs::File::create("bas64_png.txt").expect("create file failed");
    // file.write_all(bas64_png.as_ref()).expect("write failed");
    bas64_png
}

mod tests {
    use crate::resvg::render;
